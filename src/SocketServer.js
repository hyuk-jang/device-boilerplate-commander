  ...
  init() {
    const server = net
      .createServer(socket => {
        // socket.end('goodbye\n');
        console.log(
          `client is Connected ${process.env.SOCKET_PORT}\n addressInfo: ${socket.remoteAddress}`
        );
        ...
        // 인증 요청
        const authMsg = this.defaultWrapper.defaultWrapFrame({});
        BU.CLI(authMsg);
        socket.write(authMsg);

        // 데이터 수신
        stream.on('data', bufData => {
          try {
            // Parser 가 ETX 까지 삭제하므로 끝에 붙임
            bufDataStorage += Buffer.concat([bufData, this.defaultConverter.protocolConverter.ETX]);
            // BU.CLI(data);
            // 수신받은 데이터의 CRC 계산 및 본 데이터 추출
            const strData = this.defaultConverter.decodingMsg(bufDataStorage).toString();
            // BU.CLI(strData);
            ...
            errorCount += 1;
            // 기본 ETX가 메시지에 포함되어 있을 경우를 산정. 1번의 기회를 줌
            if (errorCount < 2) {
              return false;
            }
            // 에러 카운트 초기화
